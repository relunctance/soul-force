...
# =============================================================================
# Test Constants - Common Values
# =============================================================================
# Test dates
DATE_2026_04_01 = "2026-04-01"
DATE_2026_04_03 = "2026-04-03"
DATE_2026_04_04 = "2026-04-04"
DATE_2026_04_05 = "2026-04-05"
TIMESTAMP_2026_04_05 = "2026-04-05T10:00:00"
# Memory file paths
MEMORY_FILE_2026_04_01 = f"memory/{DATE_2026_04_01}.md"
MEMORY_FILE_2026_04_04 = f"memory/{DATE_2026_04_04}.md"
MEMORY_FILE_2026_04_05 = f"memory/{DATE_2026_04_05}.md"
MEMORY_SOURCES = [MEMORY_FILE_2026_04_05]
# Target file names
FILE_SOUL = "SOUL.md"
FILE_USER = "USER.md"
...
TEST_WORKSPACE_MAIN = "/tmp/test-workspace"
TEST_WORKSPACE_WUKONG = "~/.openclaw/workspace-wukong"
TEST_WORKSPACE_TSENG = "/path/to/workspace-tseng"
TEST_WORKSPACE_DEFAULT = "~/.openclaw/workspace"
# Test content
CONTENT_TEST = "Test content"
CONTENT_TEST_CONTENT = "Test content to add."
CONTENT_EXISTING = "Some existing content"
CONTENT_NEW = "This is brand new information."
# Pattern defaults
DEFAULT_PATTERN_ID = "test_001"
...
def _make_pattern(
    ...
    evidence_count=4,
    source_entries=None,
    insertion_point=DEFAULT_INSERTION_APPEND,
    auto_apply=True,
    needs_review=False,
):
    """Create a DiscoveredPattern with common defaults."""
    if source_entries is None:
        source_entries = [MEMORY_FILE_2026_04_05]
...
def test_config_defaults():
    ...
    assert FILE_SOUL in [Path(f).name for f in config.target_files]
    _pass("test_config_defaults")
def test_config_overrides():
    """Test configuration overrides."""
    config = SoulForgeConfig(overrides={
        "workspace": TEST_WORKSPACE_MAIN,
        "trigger_threshold": 5,
    })
    assert config.workspace == TEST_WORKSPACE_MAIN
...
def test_config_backup_retention():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Important files
        assert config.get_backup_retention(FILE_SOUL) == 20
        assert config.get_backup_retention(FILE_IDENTITY) == 20
        # Normal files
        assert config.get_backup_retention(FILE_USER) == 10
        assert config.get_backup_retention(FILE_MEMORY) == 10
        assert config.get_backup_retention(FILE_AGENTS) == 10
...
def test_config_last_run_timestamp():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Write a timestamp
        config.set_last_run_timestamp(TIMESTAMP_2026_04_05)
        assert config.get_last_run_timestamp() == TIMESTAMP_2026_04_05
        # Write current time
...
def test_config_agent_suffix():
    ...
    config1 = SoulForgeConfig(overrides={"workspace": TEST_WORKSPACE_DEFAULT})
    assert config1.agent_suffix == "main"
    config2 = SoulForgeConfig(overrides={"workspace": TEST_WORKSPACE_WUKONG})
    assert config2.agent_suffix == "wukong"
    config3 = SoulForgeConfig(overrides={"workspace": TEST_WORKSPACE_TSENG})
...
def test_memory_entry_creation():
    ...
    entry = MemoryEntry(
        source=MEMORY_FILE_2026_04_05,
        source_type="daily_log",
        category="conversation",
        content=CONTENT_TEST,
        timestamp=DATE_2026_04_05,
        importance=0.8,
    )
    assert entry.source == MEMORY_FILE_2026_04_05
    assert entry.source_type == "daily_log"
    assert entry.content == CONTENT_TEST
    # Test to_dict
    d = entry.to_dict()
    assert d["source"] == MEMORY_FILE_2026_04_05
...
def test_memory_reader_daily_logs():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        memory_dir.mkdir()
        test_file = memory_dir / f"{DATE_2026_04_05}.md"
        test_file.write_text(f"# {DATE_2026_04_05}\n\nTest conversation content here.")
        config = _make_config(tmpdir)
...
def test_memory_reader_incremental():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        old_file = memory_dir / f"{DATE_2026_04_01}.md"
        old_file.write_text(f"# {DATE_2026_04_01}\n\nOld content.")
        new_file = memory_dir / f"{DATE_2026_04_05}.md"
        new_file.write_text(f"# {DATE_2026_04_05}\n\nNew content.")
        config = _make_config(tmpdir)
        ...
        config.set_last_run_timestamp(f"{DATE_2026_04_03}T00:00:00")
        ...
        assert DATE_2026_04_01 not in timestamps or DATE_2026_04_05 in timestamps
...
def test_discovered_pattern_to_dict():
    ...
    d = p.to_dict()
    assert d["pattern_id"] == DEFAULT_PATTERN_ID
...
def test_discovered_pattern_from_dict():
    ...
    assert p.target_file == FILE_USER
...
def test_pattern_to_markdown_block():
    ...
    pattern = _make_pattern(
        ...
        source_entries=[MEMORY_FILE_2026_04_04, MEMORY_FILE_2026_04_05],
    )
...
def test_evolver_backup_retention_important():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        backups = list(backup_dir.glob(f"{FILE_SOUL}.*.bak"))
...
def test_evolver_backup_retention_normal():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(15):
            ...
            evolver._create_backup(target_file, backup_type="auto")
        backup_dir = Path(config.backup_dir)
        backups = list(backup_dir.glob(f"{FILE_USER}.*.bak"))
...
def test_evolver_backup_type_naming():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        backup_dir = Path(config.backup_dir)
        auto_backups = list(backup_dir.glob(f"{FILE_SOUL}.*.auto.bak"))
        manual_backups = list(backup_dir.glob(f"{FILE_SOUL}.*.manual.bak"))
...
def test_evolver_create_manual_backup():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert FILE_SOUL in backed_names
        assert FILE_USER in backed_names
